...
module Shotstack
  # The VideoAsset adds a video to a Clip. The video can be sourced from a URL (`src`), generated from a text prompt (`prompt`), or both. At least one of `src` or `prompt` must be provided.  - **Source URL:** set `src` to the URL of an mp4 (or compatible) video file. - **Generated:** set `prompt` to describe the motion. Choose a generator   with `model` and configure it with model-specific `options` (including a   starting image for image-to-video). The generated `src` is filled in   automatically. - **Both:** `src` acts as a preview placeholder while `prompt` drives   generation — the video is regenerated from the prompt at render time.   Unchanged prompts and options resolve from the generation cache. 
  class VideoAsset
    ...
    def hash
      [type, src, prompt, model, options, transcode, trim, volume, volume_effect, speed, crop, chroma_key].hash
    end
