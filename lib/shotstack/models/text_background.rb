  ...
  class TextBackground
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `Shotstack::TextBackground` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `Shotstack::TextBackground`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def list_invalid_properties
      ...
      if !@padding.nil? && @padding > 100
        invalid_properties.push('invalid value for "padding", must be smaller than or equal to 100.')
      end
    ...
    def valid?
      ...
      return false if !@padding.nil? && @padding > 100
    ...
    def padding=(padding)
      ...
      if padding > 100
        fail ArgumentError, 'invalid value for "padding", must be smaller than or equal to 100.'
      end
