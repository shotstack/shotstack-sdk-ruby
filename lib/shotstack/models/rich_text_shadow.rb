  ...
  class RichTextShadow
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `Shotstack::RichTextShadow` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `Shotstack::RichTextShadow`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }

      if attributes.key?(:'offset_x')
        self.offset_x = attributes[:'offset_x']
      else
        self.offset_x = 0
      end

      if attributes.key?(:'offset_y')
        self.offset_y = attributes[:'offset_y']
      else
        self.offset_y = 0
      end

      if attributes.key?(:'blur')
    ...
    def list_invalid_properties
      ...
      if !@blur.nil? && @blur < 0
        invalid_properties.push('invalid value for "blur", must be greater than or equal to 0.')
      end
    ...
    def valid?
      ...
      return false if !@blur.nil? && @blur < 0
    ...
    # @param [Object] blur Value to be assigned
    def blur=(blur)
      if blur.nil?
        fail ArgumentError, 'blur cannot be nil'
      end

      if blur < 0
        fail ArgumentError, 'invalid value for "blur", must be greater than or equal to 0.'
      end
    ...
    def hash
      [offset_x, offset_y, blur, color, opacity].hash
    end
