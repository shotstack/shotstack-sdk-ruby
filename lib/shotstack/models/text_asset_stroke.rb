  ...
  class TextAssetStroke
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `Shotstack::TextAssetStroke` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `Shotstack::TextAssetStroke`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def list_invalid_properties
      ...
      if !@width.nil? && @width > 10
        invalid_properties.push('invalid value for "width", must be smaller than or equal to 10.')
      end
    ...
    def valid?
      ...
      return false if !@width.nil? && @width > 10
    ...
    def width=(width)
      ...
      if width > 10
        fail ArgumentError, 'invalid value for "width", must be smaller than or equal to 10.'
      end
